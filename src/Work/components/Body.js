...
const Body = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            setRestaurantListLocal(json?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants);     // Optional Chaining for nullish checks
            setSearchResults(json?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants);     // Optional Chaining for nullish checks
            setDataLoaded(true);
        } catch (error) {
